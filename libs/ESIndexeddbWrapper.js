...
class IH {
	...
	async dQ(id) {
		...
		if (z.c < 1) {
			...
			if (z.c > 1) {
				...
				z.c--;
				z.s = void 0;
				if (z.c < 1 && z.qu.length > 0) z.dQ(id);
			} else {
				if (z.qu.length > 0) z.dQ(id);
			}
		}
	}
	dQE() {
	...
	eQ(c, d, m) {
		const seed = new Uint32Array(1),
			id = Math.floor(Math.random() * Date.now()) + seed[0];
		return pr((rv, rj) => cb(this.qu.push({ cmd: c, data: d, rv, rj, mode: m }), this.dQ(id))); //enQueue
	}
...
class IC {
	...
	async IU(tn, kpn, d, cbc = cb) {
		...
		return await (v === void 0
			? z.add(ob, k, d).catch(tE(`IU->add tn:${tn}`))
			: z.put(ob, k, d).catch(tE(`IU->put tn:${tn}`)));
	}
	async delWithRng(p) {
